...
OUTPUT_PNG = os.path.join(BASE_DIR, 'R_over_length_TM.png')
...
def load_r_stats(csv_path):
    ...
    return float(np.mean(r_values)), float(np.min(r_values))
...
def collect_r_stats_by_length():
    ...
    for name in os.listdir(BASE_DIR):
        ...
        mean_r, min_r = load_r_stats(best_csv_path)
        records.append((length, mean_r, min_r, best_fitness, best_csv_path))

    records.sort(key=lambda item: item[0])
    return records


def plot_r_over_length(records):
    lengths = [item[0] for item in records]
    mean_r_values = [item[1] for item in records]
    min_r_values = [item[2] for item in records]

    plt.figure(figsize=(10, 6))
    plt.plot(lengths, mean_r_values, 'o-', linewidth=2, markersize=6, label='Mean R')
    plt.plot(lengths, min_r_values, 's--', linewidth=2, markersize=5, label='Min R')
    plt.xlabel('Length (mm)')
    plt.ylabel('R')
    plt.title('R over Length (TM)')
    plt.grid(True)
    plt.legend()
    plt.tight_layout()
...
def main():
    records = collect_r_stats_by_length()
    if not records:
        raise SystemExit('No valid best_gen CSV files were found.')

    print('Length (mm) | Mean R | Min R | Best Fitness | CSV')
    for length, mean_r, min_r, best_fitness, csv_path in records:
        print(f'{length:>10.1f} | {mean_r:>6.4f} | {min_r:>5.4f} | {best_fitness:>12.6f} | {csv_path}')

    plot_r_over_length(records)
